...
import { useAdminCategory } from '../../hooks/admin/useAdminCategory'; // Adjust path if needed

// --- Icon Components for UI ---
const LocationPinIcon = () => <svg className="w-5 h-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fillRule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 005.16-4.057l-1.18-1.18a15.475 15.475 0 01-4.022 3.055l-2.296 1.532a1.125 1.125 0 01-1.344 0l-2.296-1.532a15.475 15.475 0 01-4.022-3.055l-1.18 1.18a16.975 16.975 0 005.16 4.057l.071.041zM12 1.5a6.75 6.75 0 100 13.5 6.75 6.75 0 000-13.5z" clipRule="evenodd" /><path d="M12 12.75a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5z" /></svg>;
const StarIcon = ({ filled }) => <svg className={`w-5 h-5 transition-colors ${filled ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>;
...
const FilterSidebar = ({ onFilterChange, initialFilters = {} }) => {
  // --- State for managing all filter values ---
  ...
  const { categories: availableCategories, isLoading: isLoadingCategories } = useAdminCategory();
  ...
  const handleApplyFilters = () => {
    const activeCategoryIds = Object.keys(selectedCategories).filter(key => selectedCategories[key]);
    onFilterChange({
        location,
  ...
  return (
    <div className="w-full max-w-sm p-6 bg-white rounded-2xl shadow-xl font-sans h-full flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto py-6 pr-2 -mr-2 space-y-6">
        {/* Location Filter */}
        <div>
            <label htmlFor="location-filter" className="text-md font-semibold text-gray-800 mb-3 block">Location</label>
        ...
        <div>
          ...
          <div className="space-y-3 max-h-40 overflow-y-auto pr-2">
            {isLoadingCategories ? <p className="text-sm text-gray-500">Loading...</p> : 
        ...
        {/* Price Range Filter */}
        <div>
          <h3 className="text-md font-semibold text-gray-800 mb-3">Max Price</h3>
          <input
            type="range"
            min="0"
            max="5000"
