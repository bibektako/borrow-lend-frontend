...
const SingleItemCard = ({ item }) => {
  ...
  const renderStars = () => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(<StarIcon key={i} className={`w-4 h-4 ${i <= itemRating ? 'text-yellow-400' : 'text-gray-300'}`} filled={i <= itemRating} />);
    }
    return stars;
  };
      ...
      <div className="w-full rounded-2xl overflow-hidden bg-white font-sans border border-gray-200 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center mt-3">
            ...
            <span className="text-gray-500 ml-2 text-xs">({reviewCount || 0} reviews)</span>
          </div>
          ...
          <div className="flex justify-between items-end mt-4">
            ...
            <div className="px-4 py-1.5 text-sm font-semibold text-blue-600 bg-blue-100 rounded-full transition-all duration-300 group-hover:bg-blue-600 group-hover:text-white">
              Borrow
            </div>
          </div>
        </div>
