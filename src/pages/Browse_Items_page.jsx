  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="w-full max-w-screen-xl mx-auto flex flex-col lg:flex-row pt-6 px-4">
        ...
        <div className={`fixed inset-0 z-40 lg:hidden transition-opacity ${isFilterOpen ? 'bg-black bg-opacity-50' : 'pointer-events-none opacity-0'}`} onClick={() => setIsFilterOpen(false)}>
            <aside 
              className={`fixed inset-y-0 left-0 w-4/5 max-w-sm bg-white z-50 p-6 overflow-y-auto transform transition-transform ${isFilterOpen ? 'translate-x-0' : '-translate-x-full'}`} 
              onClick={(e) => e.stopPropagation()}
            >
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold">Filters</h2>
                    <button onClick={() => setIsFilterOpen(false)} className="p-2 text-gray-500 hover:text-gray-800">
                        <X size={24} />
                    </button>
                </div>
                <FilterSidebar onFilterChange={handleFilterChange} />
        ...
        <main className="w-full lg:w-3/4 xl:w-4/5">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Browse Items</h1>
            <button 
          ...
          {error && (
            <div className="flex justify-center items-center h-64">
                <p className="text-red-500">{error.message || 'Failed to fetch items.'}</p>
            </div>
          )}
