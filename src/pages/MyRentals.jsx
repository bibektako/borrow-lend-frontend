...
const RequestCard = ({ request, isOwner }) => {
  ...
  const itemImage = request.item?.imageUrls?.[0]
    ? getBackendImageUrl(request.item.imageUrls[0])
    : "https://via.placeholder.com/150";

  const itemName = request.item?.name ?? "Unknown Item";
  const itemId = request.item?._id ?? "#";
  const userName = isOwner
    ? request.borrower?.username ?? "Unknown User"
    : request.owner?.username ?? "Unknown Owner";
    ...
    <div className="bg-white p-4 rounded-lg shadow-md border flex flex-col sm:flex-row gap-4">
      ...
      <div className="flex-grow">
        <div className="flex justify-between items-start">
          <div>
            <Link to={`/item/${itemId}`} className="font-bold text-lg text-gray-800 hover:underline">
              {itemName}
            </Link>
            <p className="text-sm text-gray-500">
              {isOwner ? `Requested by: ${userName}` : `Owner: ${userName}`}
            </p>
          </div>
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${statusStyles[request.status]}`}>
            {request.status}
          </span>
        </div>
  ...
  const { outgoingRequests, incomingRequests } = useMemo(() => {
    ...
    return {
      outgoingRequests: requests.filter(
        req => req.item && req.borrower && req.borrower._id === user._id
      ),
      incomingRequests: requests.filter(
        req => req.item && req.owner && req.owner._id === user._id
      ),
    };
  }, [requests, user]);
