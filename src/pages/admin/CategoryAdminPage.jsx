...
import { getCategoriesApi, updateCategoryApi,deleteCategoryApi,createCategoryApi } from '../../api/categoryAPI';
...
const CategoryPage = () => {
    ...
    const handleSaveCategory = async (categoryData, imageFile) => {
        ...
        try {
            ...
            if (status === 404) {
                 errorMessage = "Failed to save: The API endpoint was not found. Please check your server routes.";
            } else if (status === 401 || status === 403) {
                 errorMessage = "Failed to save: You are not authorized. Please log in as an admin.";
            }
            setError(errorMessage);
                                    ...
                                    {categories.map((category) => (
                                        <tr key={category._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <img className="h-12 w-12 rounded-lg object-cover" src={category.image} alt={category.name} onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/100x100/CCCCCC/FFFFFF?text=Error'; }} />
                                            </td>
